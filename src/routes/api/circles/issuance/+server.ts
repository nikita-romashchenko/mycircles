...
export const GET: RequestHandler = async ({ locals }) => {
  try {
    ...
    // Calculate issuance using direct RPC call to HubV2 contract
    const [amount, startPeriod, endPeriod] = await sdk.core.hubV2.calculateIssuance(
      userProfile.safeAddress as `0x${string}`
    )

    console.log(`Issuance for ${userProfile.safeAddress}: ${amount.toString()}`)

    return json({
      success: true,
      issuance: amount.toString(),
      startPeriod: startPeriod.toString(),
      endPeriod: endPeriod.toString(),
      address: userProfile.safeAddress,
    })
  } catch (err: any) {
    console.error("Error calculating issuance:", err)
    console.error("Error stack:", err.stack)
    return json(
      { error: err.message || "Failed to calculate issuance" },
      { status: 500 }
    )
  }
}
